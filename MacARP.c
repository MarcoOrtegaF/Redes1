...
void obtenerDatos(int ds, int * Indice){
  struct ifreq nic;
  unsigned char nombre[20] = {'w', 'l', 'p', '2', 's', '0'};    //DEJAMOS POR DEFAULT LA INTERFAZ WIFI
...
void crearTrama (unsigned char *trama){
  ...
  memcpy(trama + 12, EthertypeARP, 2);
    //MENSAJE ARP
  ...
  while (mtime < 200) {
    ...
    if(tam == -1){
      ...
      if (!memcmp(trama + 0, MACOrigen, 6) || !memcmp(trama + 6, MACOrigen, 6)) {
        ...
        if (trama[12] == 8 && trama[13] == 6){    //COMPROBAMOS PROTOCOLO ARP
          //ImpTrama(trama, tam);
          if (trama[21] == 2){                    //COMPROBAMOS QUE SEA UNA RESPUESTA
              //ImpTrama(trama, tam);
              //usleep(100);
              ImpMac(trama);
              printf("\n");
